...
void main() {
  late UnsuspendUser usecase;
  late MockUserProfileRepository repository;

  setUp(
    () {
      repository = MockUserProfileRepository();
      usecase = UnsuspendUser(repository: repository);
    },
  );

  test(
    'UserPorfile should return [Voidresult] from repository when UnsuspendUser is called',
    () async {
      // arrange
      when(() => repository.unsuspendUser(any()))
