...
void main() {
  late GetDashboardLastFiveItemsActionsStream usecase;
  ...
  setUp(
    () {
      ...
      usecase = GetDashboardLastFiveItemsActionsStream(repository: repository);
    },
  );

  group('Dashboard Inventory Actions', () {
    test(
      'should return [ItemActionsStream] from repository when GetdashboardLastFiveItemActionStream is called',
      () async {
        // arrange
        when(() => repository.getDashboardLastFiveItemActionsStream(any()))
            .thenAnswer(
          (_) async => Right(
            ItemActionsStream(
